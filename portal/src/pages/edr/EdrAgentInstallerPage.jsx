 ...
 *
 * Lets admins generate deployment tokens, copy platform-specific install
 * commands, and manage self-enrollment for Windows / Linux / macOS endpoints.
 */
...
const OS_CFG = {
  windows: { label: "Windows",    color: "#4d9eff", icon: "🪟" },
  linux:   { label: "Linux",      color: "#f5c518", icon: "🐧" },
  macos:   { label: "macOS",      color: "#b06eff", icon: "🍎" },
};
  ...
  return (
    <div style={{ padding:"28px 32px", minHeight:"100vh", background:BG }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:28 }}>
        <div>
          <h1 style={{ margin:0, fontSize:22, fontWeight:800, color:"#e8eaf0" }}>Agent Deployment</h1>
          <div style={{ fontSize:12, color:"#555", marginTop:4 }}>
            Generate deployment tokens and install CyEDR agents across your fleet
          </div>
        </div>
      ...
      <div style={{ background:CARD_BG, border:BORDER, borderRadius:12, padding:"20px 24px", marginBottom:28 }}>
        <div style={{ fontSize:14, fontWeight:700, color:"#e8eaf0", marginBottom:16 }}>Quick Deploy</div>
        <div style={{ display:"flex", gap:10, marginBottom:18, flexWrap:"wrap" }}>
          {/* Token selector */}
          <select
            value={activeToken || ""}
            onChange={e => { setActiveToken(e.target.value); if(e.target.value) loadCommands(e.target.value); }}
            style={{ background:"rgba(255,255,255,0.05)", border:BORDER, borderRadius:6, color:"#e8eaf0", padding:"7px 12px", fontSize:12, minWidth:200 }}
          >
          ...
          {Object.entries(OS_CFG).map(([os, cfg]) => (
            <button key={os} onClick={() => setSelectedOs(os)} style={{
              border:`1px solid ${selectedOs===os ? cfg.color : "#333"}`,
              borderRadius:6, background:selectedOs===os ? `${cfg.color}22` : "transparent",
              color:selectedOs===os ? cfg.color : "#666",
        ...
        {cmds && (
          <>
            <div style={{ display:"flex", gap:8, marginBottom:12 }}>
              {Object.keys(cmds[selectedOs] || {}).map(method => (
                <button key={method} onClick={() => setSelectedCmd(method)} style={{
                  border:`1px solid ${selectedCmd===method ? ACCENT : "#333"}44`,
                  borderRadius:5, background:selectedCmd===method ? `${ACCENT}22` : "transparent",
                  color:selectedCmd===method ? ACCENT : "#666",
                  padding:"4px 12px", fontSize:11, cursor:"pointer",
                }}>{method}</button>
              ))}
            </div>
            {cmds[selectedOs]?.[selectedCmd] && (
              <CopyBox value={cmds[selectedOs][selectedCmd]} label={`${OS_CFG[selectedOs].label} — ${selectedCmd}`} />
            )}
            <div style={{ fontSize:11, color:"#444", marginTop:8 }}>
              Collector URL: <code style={{ color:"#7090b0", fontFamily:"monospace" }}>{cmds.collector_url}</code>
            </div>
          </>
        )}
        ...
        {!cmds && !activeToken && (
          <div style={{ fontSize:12, color:"#555", padding:"12px 0" }}>
            Select a deployment token above, then copy the install command for your target platform.
          </div>
        )}
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(200px, 1fr))", gap:12, marginBottom:28 }}>
        {[
          { step:"1", title:"Install Agent",     desc:"Agent runs installer with deployment token",  color:"#4d9eff"  },
          { step:"2", title:"Self-Enroll",        desc:"Agent calls /api/edr/agents/self-enroll",   color:"#00e5a0"  },
          { step:"3", title:"Receive Policies",   desc:"Platform pushes assigned policies via APPLY_POLICY", color:"#b06eff"  },
      ...
      {/* ── Token list ── */}
      <div style={{ fontSize:14, fontWeight:700, color:"#e8eaf0", marginBottom:14 }}>
        Deployment Tokens
      </div>
      {loading ? (
      ...
      <div style={{ background:CARD_BG, border:BORDER, borderRadius:10, padding:"16px 20px", marginTop:24 }}>
        ...
        <div style={{ fontSize:11, color:"#666", marginBottom:8 }}>
          Admins can also enroll agents directly via the REST API without a deployment token:
        </div>
