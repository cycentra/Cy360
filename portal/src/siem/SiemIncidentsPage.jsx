...
// ── Incident drawer ────────────────────────────────────────────────────────────
function IncidentDrawer({ incident, onClose, onPatched }) {
  ...
  const handleSave = async () => {
    ...
    const data = await siemFetch(
      siemApi.patchIncident(incident.id, { status, notes, assigned_to: assignee })
    );
    setSaving(false);
    if (!data._error && !data._offline) {
      setSaved(true);
      setTimeout(() => setSaved(false), 2000);
      onPatched?.({ ...incident, status, notes, assigned_to: assignee });
    }
  };

  const misp = incident.misp_enrichment || {};
  const corrRules = incident.correlated_rules || [];
  const uebaFlags = incident.ueba_flags || [];

  return (
    <div style={{ position: "fixed", top: 0, right: 0, width: "min(580px, 95vw)", height: "100vh",
      ...
      <div style={{ padding: "20px 24px", borderBottom: "1px solid rgba(255,255,255,0.07)",
        ...
        top: 0, background: "#0d1117", zIndex: 10 }}>
        <div>
      ...
      <div style={{ padding: "20px 24px", flex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 16, marginBottom: 20, flexWrap: "wrap" }}>
          {[
            ...
            { label: "Agents", value: (incident.affected_agents || []).length },
            { label: "Users",  value: (incident.affected_users || []).length },
            { label: "Src IPs", value: (incident.src_ips || []).length },
          ].map(s => (
            <div key={s.label} style={{ background: "rgba(255,255,255,0.03)",
              border: "1px solid rgba(255,255,255,0.07)", borderRadius: 4, padding: "10px 16px" }}>
              <div style={{ color: "#00e5a0", fontSize: 18, fontWeight: 700 }}>{s.value}</div>
        ...
        {/* MITRE tags */}
        {(incident.mitre_ids || []).length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10, fontFamily: "monospace",
              letterSpacing: "1px", marginBottom: 6 }}>MITRE ATT&CK</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {(incident.mitre_ids || []).map(id => (
                <span key={id} style={{ background: "rgba(77,158,255,0.1)", color: "#4d9eff",
                  border: "1px solid rgba(77,158,255,0.3)", fontSize: 10, fontFamily: "monospace",
                  padding: "2px 8px", borderRadius: 2 }}>{id}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {corrRules.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10, fontFamily: "monospace",
              letterSpacing: "1px", marginBottom: 6 }}>CORRELATION RULES TRIGGERED</div>
            {corrRules.map((r, i) => (
        ...
        {uebaFlags.length > 0 && (
          <div style={{ marginBottom: 16 }}>
            <div style={{ color: "rgba(255,255,255,0.3)", fontSize: 10, fontFamily: "monospace",
              letterSpacing: "1px", marginBottom: 6 }}>UEBA FLAGS</div>
            {uebaFlags.map((f, i) => (
              <div key={i} style={{ display: "flex", gap: 8, marginBottom: 4, alignItems: "center" }}>
                <span style={{ color: "#ff8c00", fontSize: 10, fontFamily: "monospace" }}>⚡</span>
        ...
        {(misp.ioc_hits || []).length > 0 && (
          <div style={{ marginBottom: 16, background: "rgba(255,59,59,0.05)",
            border: "1px solid rgba(255,59,59,0.2)", borderRadius: 4, padding: "12px 16px" }}>
            <div style={{ color: "#ff3b3b", fontSize: 10, fontFamily: "monospace",
              letterSpacing: "1px", marginBottom: 8 }}>🔴 THREAT INTEL MATCHES (MISP)</div>
        ...
        {incident.llm_summary && (
          <div style={{ marginBottom: 16, background: "rgba(0,229,160,0.03)",
            border: "1px solid rgba(0,229,160,0.15)", borderRadius: 4, padding: "14px 16px" }}>
            <div style={{ color: "#00e5a0", fontSize: 10, fontFamily: "monospace",
              letterSpacing: "1px", marginBottom: 8 }}>🤖 AI NARRATIVE</div>
        ...
        <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)",
          borderRadius: 4, padding: "16px 18px", marginTop: 8 }}>
          <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 10, fontFamily: "monospace",
            letterSpacing: "1px", marginBottom: 12 }}>UPDATE INCIDENT</div>

          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
            <div>
              <label style={{ color: "rgba(255,255,255,0.4)", fontSize: 11, fontFamily: "monospace" }}>
                Status
              </label>
        ...
        {loading ? (
          ...
          <div style={{ background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.07)",
            ...
            <div style={{ display: "grid",
              gridTemplateColumns: "130px 80px 1fr 70px 80px 90px 90px",
              gap: 12, padding: "10px 16px",
              background: "rgba(255,255,255,0.03)", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
              {["ID", "SEVERITY", "AFFECTED AGENTS", "ALERTS", "STATUS", "MITRE", "LAST SEEN"].map(h => (
            ...
            {sortedIncidents.map(inc => (
              <div key={inc.id}
                ...
                <div style={{ color: "rgba(255,255,255,0.4)", fontSize: 10,
                  fontFamily: "monospace" }}>
                  {(inc.mitre_ids || []).slice(0, 2).join(", ") || "—"}
                </div>
                <div style={{ color: "rgba(255,255,255,0.35)", fontSize: 11 }}>
                  {inc.last_seen ? new Date(inc.last_seen).toLocaleTimeString() : "—"}
                </div>
              </div>
            ))}
          </div>
