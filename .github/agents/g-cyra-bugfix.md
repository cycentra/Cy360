...
Skip historical search (2 minutes max). RCA still mandatory — abbreviated format acceptable. PR targets `main` directly. Notify @g-cyra-test for expedited Suite 01 + Suite 03 + affected layer suite only. Notify @g-cyra-devops for expedited patch release tag.
