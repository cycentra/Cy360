...
def list_pdf_reports():
    ...
    Only the authenticated user's own reports (and scheduler reports) are
    returned.  Reports generated by other SSO users or by guest sessions are
    never included.
    """
    ...
    # If no uid supplied, derive from the session email (most common case).
    if not uid:
        uid = session["user_email"].strip()

    # Security: reject cross-user report access unless caller is an admin.
    session_email = session["user_email"].strip().lower()
    if uid.strip().lower() != session_email:
        try:
    ...
    # Collect PDFs from the user's own directory and shared scheduler directory.
    search_dirs = [_reports_dir_for_uid(uid)]
...
def download_pdf_report():
    ...
    # Security: only allow access to own reports (or admin cross-user).
    session_email = session["user_email"].strip().lower()
    if uid.strip().lower() != session_email:
        try:
            from blueprints.rbac.manager import get_user_role
    ...
    # Build ordered list of candidate directories: user-own dir first, then scheduler.
