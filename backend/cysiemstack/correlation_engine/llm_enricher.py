...
async def enrich_incident(db: AsyncSession, incident: Incident) -> dict:
    """Generate LLM narrative for an incident. Returns dict or None."""
    if not settings.llm_enabled:
        return {}
    ...
    try:
        ...
        if resp.status_code != 200:
            log.warning('ollama_error', status=resp.status_code, incident=incident.id)
            return {}
        ...
        log.warning('ollama_unavailable', url=settings.ollama_url)
        return {}
    except Exception as e:
        log.error('llm_enrichment_error', incident_id=incident.id, error=str(e))
        return {}
